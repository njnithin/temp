(function ($) {
  ...
  function listingTabSettings() {
    ...
    $(document).on('click', '.custom-tabs .each-tab', function (e) {
      ...
      if (!$clicked.hasClass('active')) {
        ...
        var $targetSec = $('.custom-tabs-sec').eq(parseInt($clickedValue) - 1);
        // show tab content based on click
        activateTabContent(parseInt($clickedValue) - 1);
        scrollToResults($targetSec)
      }
    });
  }
  function activateTabContent(index) {
    var $targetSec = $('.custom-tabs-sec').eq(index);
    $targetSec
      .addClass('active')
      .siblings()
      .removeClass('active');
  ...
  $(document).ajaxComplete(function () {
    var $activeTabIndex = $('.each-tab.active').attr('value');
    activateTabContent(parseInt($activeTabIndex) - 1);
  });
